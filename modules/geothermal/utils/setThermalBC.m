function [ bc ] = setThermalBC( bc, G, side, type, value, varargin )
% Impose constant temperature or heat flux boundary condition 
% on global side. Analog to setPressureBC
% 
% SYNOPSIS 
% bc = setThermalBC(bc, G, side, type, value)
% bc = setThermalBC(bc, G, side, type, value, 'pn', pv)
% bc = setThermalBC(bc, G, side, type, value, I1, I2)
% bc = setThermalBC(bc, G, side, type, value, I1, I2, 'pn', pv)
%
% PARAMETERS
% bc - Boundary condition structure as defined by function 'addThermalBC'
% ( similar to addBC )
% 
...
%              'temperature'   : fixed temperature should be in ºC 
%              'Tflux'         : temperature flux should be given in Watt 
%
...
%
% SEE ALSO:
%   'addThermalBC',  'setPressureBC'.

% Sanity check
if ~isfield(G,'cartDims'), error(msgid('NotImplemented'),'ThermalBC not implemented for this grid type'); end 
...
assert(any(isempty(sat) || any(size(sat,1)) == [1, numel(ix)]));
...
switch type 
    case 'temperature'
        ...
        bc = addThermalBC(bc, ix, 'temperature', value, 'sat', sat);
  
    case 'Tflux'
        if size(sat,1)  == 1, sat   = sat(ones([numel(ix), 1]), :); end
        a  = G.faces.areas(ix);
        sa = sum(a);
        bc = addThermalBC(bc, ix, 'Tflux', (value / sa) .* a, 'sat', sat);
