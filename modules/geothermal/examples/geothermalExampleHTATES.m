...
mrstModule add geothermal upr ad-core ad-props ad-blackoil mrst-gui
mrstVerbose on
...
%% Generate grid
% We use a grid from the <EXAMPLENAME> example in the upr module. For
% convenience, this is precomputed and stored in the module directory
data  = load('htates-reservoir.mat');
G     = data.G;
rock  = data.rock;
W     = data.W;
layer = data.layer;
...
bcT     = addThermalBC([], f(ix(1:nf)), 'temperature', T0(G.faces.centroids(f(ix(1:nf)),:)), 'sat', 1);
% Make schedule
dtPre          = rampupTimesteps(50*year, 10*year, 3);
schedulePre    = simpleSchedule(dtPre, 'bc', bc);
schedulePre.control(1).bcT = bcT;
...
fn = getPlotAfterStep(state0, model, schedule, 'useTimesteps', true); pw();
